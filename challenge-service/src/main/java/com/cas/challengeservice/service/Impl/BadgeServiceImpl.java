...
public class BadgeServiceImpl implements BadgeService {
    ...
    public GenericMessage<List<BadgeDto>> getBadgeList(BadgeGetRequest request) {
        List<Badge> matchingBadges = badgeRepository.findAllByChallengeAndUsername(request.getChallenge(), request.getUsername());
        ...
        if (matchingBadges.isEmpty()) {
            ...
            List<BadgeDto> badgeDtoList = matchingBadges.stream()
                    .map(Badge::toDto)
    ...
    public GenericMessage<BadgeDto> addBadge(BadgeAddRequest request) {
        Optional<Badge> existingBadge = badgeRepository.findByChallengeAndUsernameAndBadgeName(request.getChallenge(), request.getUsername(), request.getBadgeName());
